 ...
 * @param {string} opts.formattedAmount  — display-ready amount with currency, e.g. "₹399.00"
 * @param {string} opts.paymentId        — payment provider's transaction id (escaped)
 * @param {string} opts.formattedDate    — display-ready date string
 * @returns {{ subject: string, html: string, text: string }}
 * @throws if required params are missing
 */
export function buildLetterSealedEmail({ senderName, formattedAmount, paymentId, formattedDate } = {}) {
  if (!senderName) throw new Error('buildLetterSealedEmail: `senderName` required');
  if (!formattedAmount) throw new Error('buildLetterSealedEmail: `formattedAmount` required');
  if (!paymentId) throw new Error('buildLetterSealedEmail: `paymentId` required');
  if (!formattedDate) throw new Error('buildLetterSealedEmail: `formattedDate` required');

  const subject = LETTER_SEALED_SUBJECT;

  // Escaped variants for HTML interpolation.
  const safeSenderName = escapeHtml(senderName);
  const safeAmount = escapeHtml(formattedAmount);
  const safePaymentId = escapeHtml(paymentId);
  const safeDate = escapeHtml(formattedDate);

  // ── Type tokens (kept inline so the file remains self-contained) ──
 ...
 * @param {string} opts.formattedAmount   — pre-formatted with currency symbol, e.g. "₹399.00"
 * @param {string} opts.paymentId         — provider's payment id
 * @param {string} opts.formattedDate     — pre-formatted date string
 * @returns {Promise<{ ok: true, id: string } | { ok: false, error: string }>}
 */
export async function sendLetterSealedEmail({ to, senderName, formattedAmount, paymentId, formattedDate } = {}) {
  let built;
  try {
    built = buildLetterSealedEmail({ senderName, formattedAmount, paymentId, formattedDate });
  } catch (err) {
