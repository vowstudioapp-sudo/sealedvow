...
export const SECURITY_LIMITS = Object.freeze({
  ...
  // Firebase Storage signed URLs run 600-900 chars due to embedded RSA-256
  // signature; v4 signed URLs can reach ~1500. 2000 leaves headroom while
  // still rejecting pathological payloads. The isValidUrl host-allowlist
  // refinement remains the actual security control; this is a DoS bound.
  MAX_URL_LENGTH: 2_000,
});

// ── PRE-VALIDATION NORMALIZATION ──
// PR-46.1: optional URL fields in the form default to empty strings (e.g.
// musicUrl: '' from usePreparationState's initial state) and stay that way
// if the user never touches the corresponding input. Zod's `.optional()`
// allows the field to be ABSENT, not PRESENT-BUT-EMPTY — so '' falls through
// to `.refine(isValidUrl)`, which rejects because `new URL('')` throws.
//
// This helper, applied via `z.preprocess(emptyStringToUndefined, ...)` to
// every optional URL field, treats empty strings as absent BEFORE validation
// runs. It does NOT loosen security: malformed URLs, non-HTTPS, and non-
// allowlisted hosts still fail at the `.refine` step downstream. The only
// behavioral change is `'' → undefined → .optional() passes`.
...
const SacredLocationSchema = z.object({
  ...
  description: z.string().max(500).transform(sanitizeString),
  googleMapsUri: z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isValidUrl),
});

const MemoryPhotoSchema = z.object({
  url: z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isValidUrl),
...
export const CoupleDataSchema = z.object({
  ...
  userImageUrl: z.preprocess(emptyStringToUndefined, z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isValidUrl).optional()),
  aiImageUrl: z.preprocess(emptyStringToUndefined, z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isValidUrl).optional()),
  video: VideoDataSchema.optional(),
  audio: AudioDataSchema.optional(),

  musicType: z.enum(['preset', 'youtube']).optional(),
  musicUrl: z.preprocess(emptyStringToUndefined, z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isValidUrl).optional()),
  ...
  giftLink: z.preprocess(emptyStringToUndefined, z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isHttpsUrl).optional()),
  ...
  manualMapLink: z.preprocess(emptyStringToUndefined, z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isValidUrl).optional()),
  ...
  futureImages: z.array(z.string().max(SECURITY_LIMITS.MAX_URL_LENGTH).refine(isValidUrl)).max(10).optional(),
